...
function creatureSayCallback(cid, type, msg)
   ...
   ---------
   local need = {
      {id = 12164, qt = 200}, --screw
	  {id = 12176, qt = 25}, -- electric box
   }
   local rewards = {
      {id = 11444, qt = 1}, --thunder stone
      {id = 2160, qt = 2}, --din dim
   }
   ...
   if msgcontains(msg, 'help') or msgcontains(msg, 'ajuda') then
      ...
      selfSay("Olá meu amigo, você pode trazer para mim: 200 Screw e 25 Electric Boxes? Vou recompensá-lo!",cid)
   ...
   elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
      if getPlayerItemCount(cid, need[1].id) < need[1].qt or getPlayerItemCount(cid, need[2].id) < need[2].qt then
         ...
         selfSay("Lembre-se, você precisa trazer para mim 200 Screw e 25 Electric Boxes...", cid)
         talkState[talkUser] = 0
         return true
      end
      for i = 1, #need do
