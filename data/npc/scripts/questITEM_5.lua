...
function creatureSayCallback(cid, type, msg)
   ...
   ---------
   local need = {
      {id = 12194, qt = 100}, --future orb
	  {id = 12193, qt = 25},  -- psychic
   }
   local rewards = {
      {id = 2152, qt = 80}, --money
   ...
   if msgcontains(msg, 'help') or msgcontains(msg, 'ajuda') then
      ...
      selfSay("Hello my friend, can you bring to me: 100 future orb and 25 twisted spoons? I will reward you!",cid)
   ...
   elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
      if getPlayerItemCount(cid, need[1].id) < need[1].qt or getPlayerItemCount(cid, need[2].id) < need[2].qt then
         ...
         selfSay("Remember, you need to bring to me 100 future orb and 25 twisted spoons...", cid)
         talkState[talkUser] = 0
         return true
      end
      for i = 1, #need do
