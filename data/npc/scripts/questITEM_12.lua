...
function creatureSayCallback(cid, type, msg)
   ...
   ---------
   local need = {
      {id = 12152, qt = 1000}, --pot of lava
	  {id = 12153, qt = 500},  -- bag of pollem
   }
   local rewards = {
      {id = 2160, qt = 20}, --money
      {id = 12618, qt = 1}, --booststone
   }
   local stoFinish = 92117
   ---------
   ...
   if msgcontains(msg, 'help') or msgcontains(msg, 'ajuda') then
      ...
      selfSay("Olá meu amigo, você pode trazer para mim: 1000 Pot of Lava e 500 Bag of Pollem? Vou recompensá-lo!",cid)
   ...
   elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
      if getPlayerItemCount(cid, need[1].id) < need[1].qt then
         ...
         selfSay("Lembre-se, você precisa trazer para mim 1000 Pot of Lava e 500 Bag of Pollem...", cid)
         talkState[talkUser] = 0
         return true
      end
      for i = 1, #need do
