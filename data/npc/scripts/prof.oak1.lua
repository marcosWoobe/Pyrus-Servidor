...
local check = checkDex(cid)   
   if check >= 40 then
      if getPlayerStorageValue(cid, 748512) <= 0 then 
         selfSay("Wow i see that you have already atleast 40 pokemons in your pokedex! Take this reward!", cid)
         setPlayerStorageValue(cid, 748512, 1)
         doPlayerAddExp(cid, 100000)  --exp
         doPlayerAddItem(cid, 2160, 1)   --1 td
         doPlayerAddItem(cid, 11683, 1)  --pokeball backpack
      else
         selfSay("I already have given a reward to you...", cid)
      end
   else
      selfSay("You must have discovered at least 40 pokemons in your pokedex to take the reward!", cid)
   end    
end
