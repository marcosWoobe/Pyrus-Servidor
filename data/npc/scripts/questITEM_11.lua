...
function creatureSayCallback(cid, type, msg)
   ...
   if msgcontains(msg, 'help') or msgcontains(msg, 'ajuda') then
      ...
      selfSay("Olá meu amigo, você pode trazer para mim: 200 mimic clothes, 50 buzz tail, 500 water pendant, 200 pot of moss bug, 200 bird beaks? Vou recompensá-lo!",cid)
   ...
   elseif msgcontains(msg, 'yes') or msgcontains(msg, 'sim') and talkState[talkUser] == 1 then
      if getPlayerItemCount(cid, need[1].id) < need[1].qt then
         ...
         selfSay("Lembre-se, você precisa trazer para mim 200 mimic clothes, 50 buzz tail, 500 water pendant, 200 pot of moss bug, 200 bird beaks...", cid)
         talkState[talkUser] = 0
         return true
      end
      for i = 1, #need do
